...
abstract class BaseActiveRecord extends Model implements ActiveRecordInterface
{
    ...
    {
        if(!is_array($name) && !method_exists($this, 'get' . ucfirst($name))){
            foreach($this->_attributes as $attribute => $value){
                if((strpos($attribute, $name . '_') === 0) && !strpos($attribute, '_id')){
                    $data = new \stdClass();
                    foreach($this->_attributes as $index => $attr){
                        if(strpos($index, $name . '_') === 0){
                            $i = str_replace($name . '_', '', $index);
                            $data->$i = $attr;
                        }
                    }
